...
// THÊM FUNCTION MỚI - Batch Publish
export const publishAdsWizardBatch = async (batchData) => {
  try {
    const response = await axiosInstance.post('/api/ads-wizard/batch-publish', batchData);
    return response.data;
  } catch (error) {
    // Rethrow original axios error to preserve error.response (including error_user_msg)
    throw handleError(error);
