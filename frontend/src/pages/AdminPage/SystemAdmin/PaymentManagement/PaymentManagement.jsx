...
export default function PaymentManagement() {
  ...
  const PACKAGES_LIST = [t("common.all"), "Basic", "Pro", "Enterprise", "Starter", "Advanced"];
  const METHODS_LIST = [t("common.all"), "Momo", "VietQR", "Manual Banking"];
    ...
    try {
      ...
      if (statusFilter !== "All" && statusFilter !== allValue) {
        ...
        if (statusIndex > 0) {
          ...
          const originalStatus = originalStatuses[statusIndex - 1];
          if (originalStatus) params.status = originalStatus;
        }
      }

      // Package filter
      if (packageFilter !== "All" && packageFilter !== allValue) {
        params.package_id = packageFilter; // Backend will handle name search
      }

      // Method filter
      if (methodFilter !== "All" && methodFilter !== allValue) {
  ...
  }, [pagination.page, pagination.limit, debouncedSearch, statusFilter, packageFilter, methodFilter, dateRange, t, mapTransactionData, STATUSES]);
