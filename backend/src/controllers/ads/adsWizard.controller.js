...
export async function publishAdsWizard(req, res) {
  try {
    const { ad_account_id, page_id, access_token: tokenFromFE, campaign, adset, creative, ad } = req.body;
  ...
  const data = await publishWizard({ ad_account_id, page_id, access_token, campaign, adset, creative, ad });

    return res.status(201).json({ success: true, message: "Publish thành công.", data, logs: data.logs || [] });
  } catch (error) {
